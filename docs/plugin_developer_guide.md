...
When an HTTP 401 or 403 is returned from back-end service, it could mean the access token is expired. You can manually fetch a new VPC instance identity token, and exchange it for a new IAM access token.
...
client := vpc.NewClient(vpc.DefaultConfig(config.VPCCRITokenURL(), vpc.DefaultMetadataServiceVersion), c)
...
targetProfile := config.CurrentProfile()
profileID := targetProfile.ID
// prepare IAM token request using the existing targeted profile. See https://godoc.org/github.com/IBM-Cloud/ibm-cloud-cli-sdk/bluemix/authentication/vpc# NewIAMAccessTokenRequest for API details
...
// optional, set access token back to config
ret := fmt.Sprintf("%s %s", iamToken.TokenType, accessToken)
config.SetIAMToken(ret)
```

For more details of the API, refer to the docs for [vpc](https://godoc.org/github.com/IBM-Cloud/ibm-cloud-cli-sdk/bluemix/authentication/vpc)

_Note_: Currently a refresh token is not supported when authenticating as a VPC compute resource identity.
